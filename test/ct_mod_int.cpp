...
TEST(Construction, Default) {
  ntlib::ct_mod_int<uint32_t, MOD> a;
  EXPECT_EQ(a.get(), 0);
}

TEST(Construction, ByValue) {
  ntlib::ct_mod_int<uint32_t, MOD> a(0);
  EXPECT_EQ(a.get(), 0);

  ntlib::ct_mod_int<uint32_t, MOD> b(MOD - 1);
  EXPECT_EQ(b.get(), MOD - 1);

  ntlib::ct_mod_int<uint32_t, MOD> c(MOD);
  EXPECT_EQ(c.get(), 0);

  ntlib::ct_mod_int<int32_t, MOD> d(-MOD);
  EXPECT_EQ(d.get(), 0);

  ntlib::ct_mod_int<int32_t, MOD> e(1 - MOD);
  EXPECT_EQ(e.get(), 1 % MOD);
}

TEST(Streaming, Insertion) {
  ntlib::ct_mod_int<uint32_t, MOD> a(5), b(MOD - 1), c(MOD);
  std::string result =
...
TEST(Streaming, Extraction) {
  ...
  ntlib::ct_mod_int<uint32_t, MOD> a, b, c;
  in >> a >> b >> c;
...
TEST(Decrement, Prefix) {
  ...
  ntlib::ct_mod_int<uint32_t, MOD> b(1);
  EXPECT_EQ((--b).get(), 0);
  EXPECT_EQ(b.get(), 0);
}
...
TEST(Addition, Compound) {
  ntlib::ct_mod_int<uint32_t, MOD> a(0), b(1);
  EXPECT_EQ((a += b).get(), 1 % MOD);
  EXPECT_EQ(a.get(), 1 % MOD);
}

TEST(Addition, Symmetric) {
  ntlib::ct_mod_int<uint32_t, MOD> a(1), b(2);
  EXPECT_EQ((a + b).get(), (b + a).get());
}

TEST(Addition, Chained) {
  ntlib::ct_mod_int<uint32_t, MOD> a(1), b(2), c(3);
  EXPECT_EQ((a + b + c).get(), 6 % MOD);
}

TEST(Addition, WithInts) {
  ntlib::ct_mod_int<uint32_t, MOD> a(1);
...
TEST(Subtraction, Compound) {
  ...
  ntlib::ct_mod_int<uint32_t, MOD> c(MOD - 1), d(1);
...
TEST(Multiplication, Compound) {
  ntlib::ct_mod_int<uint32_t, MOD> a(2), b(3);
...
TEST(Multiplication, Chained) {
  ntlib::ct_mod_int<uint32_t, MOD> a(2), b(3), c(4);
  EXPECT_EQ((a * b * c).get(), 24 % MOD);
}

TEST(Multiplication, WithInts) {
  ntlib::ct_mod_int<uint32_t, MOD> a(2);
